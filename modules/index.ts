...
const getModules = (language: 'zh' | 'en' = 'zh'): DesignModule[] => {
  const t = translations[language].modules;
  
  return [
    {
      id: 'AI-rendering',
      title: t.aiRendering.title,
      description: t.aiRendering.description,
    ...
    {
      id: 'Node-Scene',
      title: t.nodeScene.title,
      description: t.nodeScene.description,
    ...
    {
      id: 'Analysis Diagram',
      title: t.analysisDiagram.title,
      description: t.analysisDiagram.description,
      imageUrl: '/modules/analysisdiagram.jpg',
      fixedPrompt: '横向长图幅时间轴数据可视化，白色工程网格纸（Graph Paper）底纹。三段式排版：顶部大标题，中部核心拼贴画卷，底部线性时间轴与说明栏。专业的建筑拼贴画风格（Architecttural Collage）,混合媒介。整体采用低纯度、柔和的色调（Muted Tones），不鲜艳。左侧为暗淡的赭石色怀旧感，右侧为低饱和的现代冷赭石色科技感（）。画面中包含赭石色半透明圆形高光，以及贯穿建筑顶部的黑色虚线共形连接线。',
    ...
    },
    {
      ...
      title: t.explodedViewDiagram.title,
      description: t.explodedViewDiagram.description,
    ...
    },
    {
      ...
      title: t.figureGroundDiagram.title,
      description: t.figureGroundDiagram.description,
    ...
    },
    {
      ...
      title: t.renovationDesign.title,
      description: t.renovationDesign.description,
...
export const MODULES: DesignModule[] = getModules('zh');
export { getModules };
